// Numbers can be added to the array to increse sum and change the text output.
...
void setup() {
  ...
  // Calls a custom method to display the name and sum of numbers.
  displayInfo(name, numbers);
}

void draw() {
  // A for-each loop to draw circles based on the numbers array.
  for (int num : numbers) {
...
void displayInfo(String name, int[] nums) {
  ...
  // and boolean operators to provide even further additional information 
  // based on the sum.
  if (sum < 50 && sum > 10) {
...
// A custom method to calculate the sum of an integer array and return it.
